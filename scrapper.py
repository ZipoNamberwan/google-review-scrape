...
def csv_writer(outpath):
    ...
    last_row = 0
    targetfile = open(csv_path, mode='a' if file_exists else 'w',
...
def stats_writer():
    ...
    if not stats_exists:
        writer.writerow(
            ['place', 'batch_start', 'batch_end', 'batch_size', 'seconds'])
...
def main():
    ...
    parser.add_argument(
        '--config', type=str, default='places.json', help='Path to places config JSON')
    ...
    for place in places:
        ...
        if error == 0:
            ...
            if n > 0:
                print(
                    colored(f"Loading reviews until {n} items are shown...", "yellow"))
                scraper.load_until_count(n)
                print(
                    colored(f"Loaded {n} reviews. Starting to save new reviews...", "green"))
            while n < review_limit:
                try:
                    target = min(n + BATCH_SIZE, review_limit)
                    print(
                        colored(f"Loading reviews until {target} items are shown...", "yellow"))
                    batch_start_time = time.time()
                    scraper.load_until_count(target)
                    print(
                        colored(f"Loaded {target} reviews. Expanding and parsing batch...", "green"))
                    ...
                    stats_writer_obj.writerow(
                        [name, n, n + len(reviews) - 1, len(reviews), f"{batch_elapsed:.2f}"])
                    stats_file.flush()
                    n += len(reviews)
